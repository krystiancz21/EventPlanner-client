...
import { updateWorkshop, getWorkshop, type UpdateWorkshopDto, type Workshop } from '@/lib/api/workshops';
...
export default function EditWorkshopForm({ workshopId }: EditWorkshopFormProps) {
  ...
  useEffect(() => {
    const fetchWorkshop = async () => {
      try {
        ...
        setCapacity(workshopData.capacity);
        
        // Format date for datetime-local input
        const formattedDate = new Date(workshopData.date).toISOString().slice(0, 16);
        setDate(formattedDate);
        
      } catch (error) {
        console.error('Error fetching workshop:', error);
        setGeneralError('Nie udało się załadować danych warsztatu');
        toaster.create({
          title: 'Błąd',
          description: 'Nie udało się załadować danych warsztatu',
          type: 'error',
          duration: 5000,
        });
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toaster.create({
        title: 'Warsztat zaktualizowany',
        description: 'Zmiany zostały pomyślnie zapisane.',
        type: 'success',
        duration: 5000,
      });
      
      // Redirect to my workshops page
      router.push('/workshops/my');
      
    } catch (error) {
      console.error('Error updating workshop:', error);
      
      setGeneralError('Wystąpił błąd podczas aktualizacji warsztatu. Spróbuj ponownie.');
      
      toaster.create({
        title: 'Błąd',
        description: 'Nie udało się zaktualizować warsztatu. Spróbuj ponownie.',
        type: 'error',
        duration: 5000,
      });
    } finally {
  ...
  // Function to format date to YYYY-MM-DD for input min value
    ...
    return (
      <Box textAlign="center" py={8}>
        <Text color="red.500" fontSize="lg">
          Nie udało się załadować danych warsztatu
        </Text>
      </Box>
    );
  }
      ...
      <Stack gap={6}>
        ...
        <Box>
          <Button
            ...
            size="lg"
          >
            {isLoading ? 'Zapisywanie zmian...' : 'Zapisz zmiany'}
          </Button>
          ...
          <Button
            ...
            variant="outline"
            width="100%"
            onClick={() => router.push('/workshops/my')}
            disabled={isLoading}
            size="lg"
          >
            Anuluj
